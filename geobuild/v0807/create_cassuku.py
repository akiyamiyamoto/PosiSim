...
#       "InShld0a  6 +zbound2 -zbound1 +rcyl3 -rcyl2 " + " ".join(rexclude[1:]) + " ".join(rexclgap[1:]) + " " + rextar,
#       "InShld0b  6 +zbound3 -zbound2 +rcyl3 -rcyl2 " + " ".join(rexclude[1:]) + " ".join(rexclgap[1:]) + " " + rextar,
#       "InShld0c  6 +z3inpln1 -zbound3 +rcyl3 -rcyl2 " + " ".join(rexclude[1:]) + " ".join(rexclgap[1:]) + " " + rextar,
#       "InShld0d  6 +zbound5 -z3inpln1 +rcyl3 -rcyl2 " + " ".join(rexclude[1:]) + " ".join(rexclgap[1:]) + " " + rextar,
...
#        "InShldf  6 +zbound2 -zbound1 +rcyl1 -rcylin " ,
       "InShldua  6 +zbound3  -zbound2 +rcyl1 -rcylin " + rextar,  
#       "InShldub  6 +z3inpln1 -zbound3 +rcyl1 -rcylin " + " ".join(rexclude[1:]) + " ".join(rexclgap[1:]) ,   
...
#            "ASSIGNMA %10s%10s" % ("CONCRETE", "InShldf"),
           "ASSIGNMA %10s%10s" % ("CONCRETE", "InShldua") + upward1,
#           "ASSIGNMA %10s%10s" % ("CONCRETE", "InShldub") + upward2,
#           "ASSIGNMA %10s%10s" % ("CONCRETE", "InShld0a"),
#           "ASSIGNMA %10s%10s" % ("CONCRETE", "InShld0b") + upward1, 
#           "ASSIGNMA %10s%10s" % ("CONCRETE", "InShld0c") + upward2,
#           "ASSIGNMA %10s%10s" % ("CONCRETE", "InShld0d")]
           ]
#    for k in ["a", "b"]:
#        assignma += [
...
#                 "Z1CSh0   6 +z1cs0end -z1cs0bgn +rcylin -rcylbpou"
       ...
       region += [ "VCCTAR 6 +rcyl1 -tvcri +( " + "|".join(vcc) + " )",
                   "VCWTAR 6 +rcyl1 -tvcri +( " + "|".join(vcw) + " ) -( " + "|".join(vcc) + ") "]
...
# ========================================
def crCassuku(geo, fd):

    # global _body, _region, _assignma

    zdata = [ geo["bases"]["zbound2"] - geo["cassuku"]["blkhole"] - geo["cassuku"]["ironshield"], 
              geo["bases"]["zbound2"] - geo["cassuku"]["blkhole"],
              geo["world"]["zbound3"] + geo["cassuku"]["blkhole"], 
              geo["world"]["zbound3"] + geo["cassuku"]["blkhole"] + geo["cassuku"]["ironshield"] ] 
              
    rcyl1 = geo["global"]["CShIn_rmin"] + geo["global"]["CShIn_thick"] 
    rdata = [ rcyl1 + geo["cassuku"]["blkhole"],
              rcyl1 + geo["cassuku"]["blkhole"] + geo["cassuku"]["ironshield"] ]

    # ############################################
    body = ["*  Body for Cassuku" ]
    for i in range(0, len(zdata)):
         body.append("XYP cazpln%d %f" % ( i+1, zdata[i] ))

    for i in range(0, len(rdata)):
         body.append("ZCC carcyl%d 0.0 0.0 %f" % ( i+1, rdata[i] ))

    # ############################################
    #     "CAssuku   6 ( +zbound5 -zbound1 +rbound3 -rcyl1 ) | ( +zbound2 -zbound1 +rcyl1 ) | ( +zbound5 -zbound3 +rcyl1 ) ",
    region = ["** region for Cassuku", 
         "CAAir     6 ( +zbound5 -zbound1 +rbound3 -carcyl2 ) | ( +zbound5 -cazpln4 +carcyl2 ) | ( +cazpln1 -zbound1 +carcyl2 ) ",

...
def createGeoParam():
    ...
    geo["bases"] = {"Target_to_FC_gap": 0.2,  # Target to FC gap
		 ...
		 "zbound2":-55.0, # Boundary between front(upstream) part and target & RF area
                 "zmin":-200.0, # zmin of whole area
                 "zmax":1000.0, # zmax of whole area
                 "rmax":200.0 } # rmax of whole area
    
#                 "shield_rin": 120.0 } # Inner radius of inner concrete shield

    # Geometry for cassuku
    geo["cassuku"] = {"blkhole": 1.0,  # Thickness of blackhope covering target body
                      "ironshield": 20.0,  # Thickness of iron case,
                      "airoutside": 100.0 }  # Thickness of air covering cassuku                  
    cassuku_thick = geo["cassuku"]["blkhole"] + geo["cassuku"]["ironshield"] + geo["cassuku"]["airoutside"]
    geo["bases"]["zmin"] = geo["bases"]["zbound2"] - cassuku_thick
    # geo["bases"]["rmax"] -= cassuku_thick

#    geo["global"]["rmax"] = geo["bases"]["rmax"]
#
...
#        "CShIn0_thick":70, # Thickness of tunnel concrete, inner
        ...
        "CShIn0_thick":170, # Thickness of tunnel concrete, inner
        "CShIn0_rmin":210.0,  # inner radius of smaller concrete sheild
        "CShIn_thick":30, # Thickness of tunnel concrete, inner
        "CShIn_rmin":110.0,  # inner radius of smaller concrete sheild
...
#        "FeSh_thick": 20.0, # Thickness of Iron sheild inside of concrete sheild
        "FeSh_thick": 10.0, # Thickness of Iron sheild inside of concrete sheild
...
#
    ...
    glp = geo["global"]
    if "cassuku" in geo:
       rmax = glp["CShIn_rmin"] + glp["CShIn_thick"] + cassuku_thick
       geo["global"]["rmax"] = rmax
       geo["bases"]["rmax"] = rmax

    #@@ world
    geo["world"] = {"zbound1":glp["zmin"], 
		     "zbound2":geo["bases"]["zbound2"], # Boundary between front(upstream) part and target & RF area
    ...
    geo["RF"] = {"z_rf_begin": zbound3 + geo["bases"]["Collimator_thickness"], # Z_begin of 1st cavity
                 ...
                 "Nb_structure":0, # Number of RF structure
    ...
    geo["Holes"]["wave_guides"] = {
                     ...
                     "zoffset": [ 0.0,52.0, 58.0 ] } # offset in Z direction, when wave guide was bended. 
    ...
    geo["Holes"]["cables"] = {"radius":1.0, # Radius of cable
                     ...
                     "zcenter_solenoid":120.0} # Z center for Solenoid power cable
    ...
    if "cassuku" in geo:
       gwp["zbound5"] = zbound3 + cassuku_thick
    else:

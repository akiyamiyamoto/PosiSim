...
def crRotationTarget(geo, fd):
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TFC","","","") ]
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TFCC","","","") ]
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TFCU","","","") ]
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("TARMAT", "TWdisk","","","") ]
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TCudisk","","","") ]
    ...
    assignma += [ "ASSIGNMA %10s%10s%10s%20s%10s%20s" % ("WATER", "TRDiskcp", "TRAxiscp","","","") ]

    region += ["TRDisk 6 +trdisk -trdiskcp -traxiscp "]
    # assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TRDisk","","VACUUM","beamoff2") ]
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TRDisk","","","") ]
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("STAINLES", "TRAxis","","","") ]
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("STAINLES", "TRotSup","","","") ]
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("LIGHTSUS", "TRotBody","","","") ]
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("LIQSEAL", "LiqSeal","","","") ]
...
def crSupportStructure(geo, fd, envelops):
    ...
    '''
    region += [ "CBFC2    6 -zbound2 +tfcubgn +cbfc",
                "CBgFC2   6 -tshbgn -tvcbo +tfcubgn +cbgfc -cbfc",
                "WLFC2    6 -zbound2 +tfcubgn +wlfc",
                "CBrot2   6 -zbound2 -trotbody +tshbgn +cbrot",
                "WLrot2   6 -zbound2 -trotbody +tshbgn +wlrot"] 
    ...
    '''
    assignma += [ "ASSIGNMA %10s%10s" % ("AIR", "CBgFC2") + beamoff2,
                  "ASSIGNMA %10s%10s" % ("Copper", "CBFC2") + beamoff2,
                  "ASSIGNMA %10s%10s" % ("Copper", "CBrot2") + beamoff2,
                  "ASSIGNMA %10s%10s" % ("WATER", "WLrot2") + beamoff2,
                  "ASSIGNMA %10s%10s" % ("WATER", "WLFC2") + beamoff2]
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("STAINLES", "TVCwall","","","") ]
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("STAINLES", "TVCwalld","","","") ]
    ...
    zbp0_bgn = gtar["vacuum_chamber_R_z_begin"] - gtar["WShield_thickness"] 
    zbp0_len = gtar["WShield_thickness"] + vc_thick
    body += ["RCC tvcbpo 0.0 0.0 %f 0.0 0.0 %f %f" % ( zbegin, bp_zlen, glbal["BPrin"] + glbal["BPthick"] ) ]
    body += ["RCC tvcbpi 0.0 0.0 %f 0.0 0.0 %f %f" % ( zbp0_bgn, zbp0_len, glbal["BPrin"] ) ]
    body += ["RCC tvcbpj 0.0 0.0 %f 0.0 0.0 %f %f" % ( zbegin, bp_zlen - zbp0_len, glbal["BPrin"] ) ]
    ...
    region += ["TVCbpvad 6 +tvcbpj"]
    region += ["TVCbpwal 6 +tvcbpo -tvcbpi - tvcbpj"]
    # region += ["TBPShld 6 +tbpshld -tvcbpo"]
    # assignma +=  [ "ASSIGNMA %10s%10s" % ("VACUUM", "TVCbpvac") ]
    assignma +=  [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("VACUUM", "TVCbpvac","","STAINLES","") ]
    assignma +=  [ "ASSIGNMA %10s%10s" % ("VACUUM", "TVCbpvad") ]
    # assignma +=  [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("STAINLES", "TVCbpwal","","VACUUM","beamoff2") ]
    assignma +=  [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("STAINLES", "TVCbpwal","","","") ]
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TWShield","","","") ]
    ...
    assignma += [ "ASSIGNMA %10s%10s%30s%10s%20s" % ("Copper", "TWShld2","","","") ]     

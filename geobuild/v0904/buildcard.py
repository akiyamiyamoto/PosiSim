...
# ====================================================
def DCYSCORE_DOSE(dcyname, postname, dcyind, unit, rmax, zmax, zmin, nbinR, nbinZ, rmin=0.0, scoretype="DOSE-EQ", mesh="11.0" ):
    ...
    card1 = "USRBIN".ljust(10)+mesh.rjust(10) + scoretype.rjust(10)
...
def DCYSCORE_ACTIVITY(dcyname, postname, dcyind, unit, rmax, zmax, zmin, nbinR, nbinZ, 
                      scoretype = "ACTIVITY", xoffset=0.0, phibin=1.0 ):
...
# ====================================================================================
def Primary_Score(name, stype, unit, rmax, zmax, zmin, nbinR, nbinF, nbinZ, rmin=0.0, mesh="11.0"):

    zero = 0.0
    card = []
    card.append("*")
    card.append("USRBIN".ljust(10)+mesh.rjust(10) + stype.rjust(10))
...
def decay_score(geodata):
    ...
    # def DCYSCORE_DOSE(dcyname, postname, dcyind, unit, 
    #   rmax, zmax, zmin, nbinR, nbinZ, rmin=0.0, scoretype="DOSE-EQ", mesh="11.0" ):
    ...
    for ind in range(0, len(decaytimes)):
       cards += DCYSCORE_DOSE(decaytimes[ind], "All", ind+1, 71, 
                rmax, zmax, zmin, par_all[3], par_all[4])
    ...
    for ind in range(0, len(decaytimes)):
       cards += DCYSCORE_ACTIVITY(decaytimes[ind], "AM", ind+1, 74, 
                rmax, zmax, zmin, par_all[3], par_all[4], scoretype="ACTOMASS")

    dcyind = 2
    # cards += DCYSCORE_DOSE(decaytimes[ind], "All", ind+1, 73, 
    #             par_all[0], par_all[1], par_all[2], par_all[3], par_all[4], mesh="0.0")
    xmax = 80.0 
    ymax = 80.0
    zmax = 50.0
    xmin = -50.0
    ...
    cards.append("USRBIN".ljust(10)+"10.0".rjust(10) + "DOSE-EQ".rjust(10))
    cards[-1] += "-73.0".rjust(10)
    cards[-1] += "%10.1f%10.1f%10.1f%s" % ( xmax, ymax, zmax, name)
    cards.append("USRBIN".ljust(10))
    cards[-1] += "%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f &" % (xmin, ymin, zmin, nbinx, nbiny, nbinz)

    cards.append( "DCYSCORE".ljust(10) )
    cards[-1] += "%10.1f%20s%10s%10s%10s" % ( dcyind, "", name, name, "") 
    cards[-1] += "USRBIN".ljust(10)

    zmin = -10.0
    zmax =  10.0
    nbinz = 20.0   
    name = "edep"
    mesh = "10.0"  # 11 for r-phi-z, 10 for x-y-z plot"
    # decaytimes = ["1h","1d", "4d", "1m","1y","Xy"]
    for dcyind in range(0, len(decaytimes)):
       name = "dose%s" % decaytimes[dcyind]
       cards.append("* Userbin Energy deposit for decay period of " + name)
       cards.append("USRBIN".ljust(10)+mesh.rjust(10) + "DOSE".rjust(10))
       cards[-1] += "-78.0".rjust(10)
       cards[-1] += "%10.1f%10.1f%10.1f%s" % ( xmax, ymax, zmax, name)
       cards.append("USRBIN".ljust(10))
       cards[-1] += "%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f &" % (xmin, ymin, zmin, nbinx, nbiny, nbinz)
       cards.append( "DCYSCORE".ljust(10) )
       cards[-1] += "%10.1f%20s%10s%10s%10s" % ( float(dcyind), "", name, name, "") 
       cards[-1] += "USRBIN".ljust(10)
...
#    mesh = "1.0"
#    for dcyind in range(0, len(decaytimes)):
#       name = "am%s" % decaytimes[dcyind]
#       cards.append("* Userbin ACtomass for decay period of " + name)
#       cards.append("USRBIN".ljust(10)+mesh.rjust(10) + "ACTIVITY".rjust(10))
#       cards[-1] += "-79.0".rjust(10)
#       cards[-1] += "%10.1f%10.1f%10.1f%s" % ( xmax, ymax, zmax, name)
#
#       cards.append("USRBIN".ljust(10))
#       cards[-1] += "%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f &" % (xmin, ymin, zmin, nbinx, nbiny, nbinz)
#
#       cards.append("DCYSCORE".ljust(10))
#       cards[-1] += "%10.1f%20s%10s%10s%10.1f" % ( float(dcyind), " ", name, name, len_step)
#       cards[-1] += "USRBIN".ljust(10)
#
#       cards.append("AUXSCORE".ljust(10) + "USRBIN".rjust(10))
#       cards[-1] += "ALL-PART".rjust(10)+ " ".rjust(10)
#       cards[-1] += name.rjust(10) + name.rjust(10)
#       cards[-1] += "%10.1f%10s" % ( len_step, " ")


    #   cards += DCYSCORE_DOSE(decaytimes[ind], "edep", ind+1, 78, 
    #            60.0, 10.0, -10.0, 60, 20, rmin=0.0, scoretype="DOSE")
    #   cards += DCYSCORE_DOSE(decaytimes[ind], "edep", ind+1, 79, 
    #            60.0, 2.0,    0.0, 60, 20, rmin=0.0, scoretype="DOSE")
#     for ind in range(0, len(decaytimes)):
...
#    cards.append(usrbin_fmt2 % ("USRBIN", 0.0, xoffset, par_tar[2], par_tar[3], phibin, par_tar[4], " &"))
    cards.append("* USERBIN for variables arround target.")
...
#    rmax_all = geo["bases"]["rmax"]
#    zmin_all = geo["bases"]["zmin"]
#    zmax_all = geo["bases"]["zmax"]
#    nbinZ = zmax_all - zmin_all
#    nbinR = rmax_all
...
#   par_all = [xmax, ymax, zmax, xmin, ymin, zmin, nxbin, nybin, nzbin]
#    cards += Primary_Score("priAll", "DOSE-EQ", 84, 
#                par_all[0], par_all[1], par_all[2], par_all[3], 200.0, par_all[4], mesh="0.0")
    xmax = 150.0 
    ymax = 150.0
    zmax = 50.0
    xmin = -50.0
    ...
    cards.append("USRBIN".ljust(10)+"10.0".rjust(10) + "DOSE-EQ".rjust(10))
    cards[-1] += "-84.0".rjust(10)
    cards[-1] += "%10.1f%10.1f%10.1f%s" % ( xmax, ymax, zmax, "priXYZ")
    cards.append("USRBIN".ljust(10))
    cards[-1] += "%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f &" % (xmin, ymin, zmin, nbinx, nbiny, nbinz)

...
print_template_plot(){
  ...
  plottitle=$3
  cat <<EOF
  $plotcmd   "${plotfile}" using ((\$1+\$2)/2*1000.0):(\$3*(\$2-\$1)*${NORM}):(0.0):(\$3*\$4*(\$2-\$1)*1.0E-5*${NORM})  with xyerrorlines title "${plottitle}"
EOF
}
...
set output "figs/flux_npart.png"
replot
...
gpl_all_usrbdx(){
    ...
    echo "${outfile} was created. Run gnuplot44 to create figure."

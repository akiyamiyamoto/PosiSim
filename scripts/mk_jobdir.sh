...
nrun_end=2
opts=""
sourcedir="${FLUKA}/init_fluka93.sh"
fluka_inp=""
jobname_key=""
  ...
  for jseq in `seq ${njob_begin} ${njob_end}`; do
    ( 
       ...
       ( 
          ...
          # for fluka IPNF version, cmd="rflukadpm3 ${opts} -N${nrun_begin} -M${nrun_end} ${thisinp}"
          cmd="rfluka -d ${opts} -N${nrun_begin} -M${nrun_end} ${thisinp}"
